...
void LogV(const char * module, uint8_t category, const char * msg, va_list v)
{
    ...
    printf("[%" PRIu64 ".%06" PRIu64 "][%ld:%ld] CHIP:%s: ", static_cast<uint64_t>(tv.tv_sec), static_cast<uint64_t>(tv.tv_usec),
           static_cast<long>(syscall(SYS_getpid)), static_cast<long>(syscall(SYS_gettid)), module);
    vprintf(msg, v);
    printf("\n");
    fflush(stdout);
